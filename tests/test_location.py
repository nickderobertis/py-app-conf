...
def test_config_location_linux(model_object: BaseConfig):
    assert model_object.settings.config_location == Path('~').expanduser() / '.config' / 'MyApp' / 'config'


def test_custom_config_location():
    expect_path = Path('/woo')
    obj = get_model_object(settings=AppConfig(app_name='MyApp', custom_config_path=expect_path))
    assert obj.settings.config_location == expect_path
